...
import { 
  Card, CardContent, Typography, Tabs, Tab, Select, MenuItem, 
  TextField, Button, Box, FormControlLabel, Checkbox, Paper, 
  Container, FormControl, InputLabel, CircularProgress, Snackbar, 
  Alert, Drawer, List, ListItem, ListItemText, ListItemSecondaryAction,
  IconButton, Slider, Divider, Dialog, DialogTitle, DialogContent,
...
const AIWriter = () => {
  ...
  const templates = {
    public: [
      { 
        ...
        prompt: 'Create a formal policy announcement speech with clear directives and implementation guidelines'
      },
      { 
        id: 'public_address', 
        name: 'Public Address',
        prompt: 'Generate an official public address focusing on government initiatives and public welfare'
      },
      { 
        id: 'inaugural_speech', 
        name: 'Inaugural Speech',
        prompt: 'Develop an inaugural speech highlighting vision, objectives, and strategic plans'
      },
    ],
    private: [
      { 
        ...
        prompt: 'Create a comprehensive departmental briefing covering progress, challenges, and action items'
      },
      { 
        id: 'committee_address', 
        name: 'Committee Address',
        prompt: 'Prepare a formal committee address discussing policy implications and recommendations'
      },
      { 
        id: 'executive_summary', 
        name: 'Executive Summary',
        prompt: 'Generate an executive summary presentation with key findings and strategic recommendations'
      },
    ]
  };
  ...
  const loadSavedSpeeches = () => {
    const speeches = JSON.parse(localStorage.getItem('bureaucraticSpeeches') || '[]');
  ...
  const generateContent = async () => {
    ...
    try {
      const template = [...templates.public, ...templates.private]
...
`;
  ...
  const handlePasswordSubmit = () => {
    ...
    if (speech && passwordDialog.passwordAttempt === speech.password) {
      loadSpeechContent(speech);
      setPasswordDialog({ open: false, speechToLoad: null, passwordAttempt: '' });
    } else {
  ...
  const handleSave = () => {
    ...
    try {
      ...
      const existingIndex = updatedSpeeches.findIndex(
        speech => speech.topic === prompt && speech.template === selectedTemplate
      );
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <CardContent>
          ...
          <Box sx={{ mb: 4 }}>
            <FormControl fullWidth sx={{ mb: 3 }}>
              ...
              >
                {templates[tabValue === 0 ? 'public' : 'private'].map(template => (
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Typography variant="caption" color="text.secondary">
                Approximate word count: {calculateWordsForDuration(speechDuration)} words
              </Typography>
            </Box>
          ...
          <Box sx={{ display: 'flex', gap: 2, mb: 4 }}>
            <Button
              variant="contained"
              startIcon={isGenerating ? <CircularProgress size={20} color="inherit" /> : <CreateIcon />}
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            This speech is password protected. Please enter the password to view and edit.
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
